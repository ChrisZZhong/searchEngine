...
def processing(document, indexType):
    ...
    if indexType == 'positional':
        ...
        tokens = constructPhrases(tokens)
        return tokens
    else:
        tokens = removeStop(tokens)
        return tokens
...
def processingPipLines(token, indexType):
    """This function handles passing an individual token
    to every function to be processed."""

    token = extractFile(token, indexType)
    token = extraZeros(token, indexType)
    token = startHeading(token)
    token = deleteNums(token, indexType)
    token = currency(token, indexType)
    token = periodEnd(token)
    token = periods(token, indexType)
    return token


def deleteNums(token, indexType):
...
def extractFile(token, indexType):
...
def extraZeros(token, indexType):
...
def deleteEmpty(tokens):
    tokens = [token for token in tokens if token]
    return tokens


def splitSpecialChar(text, indexType):
...
def stemming(tokens):
    ...
    for token in tokens:
        try:
            ...
            stemmed.append('')
    stemmend = deleteEmpty(stemmed)
    return stemmed


def constructPhrases(tokens):
    stopWords = stopList()
    special_case = 'STOP'
    string_2 = ''
    string_3 = ''
